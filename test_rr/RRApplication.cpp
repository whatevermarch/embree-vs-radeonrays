...
void RRApplication::Run(uint32_t res, MeshData* mesh_data, std::vector<uint32_t>& out_data)
{
	...
	for (int x = 0; x < res; ++x)
	{
		for (int y = 0; y < res; ++y)
		{
			...
			rays[i].origin[0] = 0.f;
			rays[i].origin[1] = 250.f;
			rays[i].origin[2] = 0.f;

			rays[i].direction[0] = -1.f;
			rays[i].direction[1] = -1.f + (2.f / res) * y;
			rays[i].direction[2] = -1.f + (2.f / res) * x;
	...
	size_t scratch_size;
	rrGetTraceMemoryRequirements(context, res * res, &scratch_size);
...
inline VkScopedObject<VkDeviceMemory> RRApplication::AllocateDeviceMemory(std::uint32_t memory_type_index,
...
inline std::uint32_t RRApplication::FindDeviceMemoryIndex(VkMemoryPropertyFlags flags) const
{
	...
	for (auto i = 0u; i < mem_props.memoryTypeCount; i++)
	{
		...
		if ((memory_type.propertyFlags & flags) == flags)
		{
			...
			//{
			//	std::assert(memory_type.propertyFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
			//}
			return i;
		}
	}
...
inline VkScopedObject<VkBuffer> RRApplication::CreateBuffer(VkBufferUsageFlags usage, std::size_t size) const
{
...
inline void RRApplication::UploadMemory(std::vector<TYPE> const& source, VkBuffer const& destination) const
{
